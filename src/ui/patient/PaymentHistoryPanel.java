...
import java.awt.*;
    ...
    public class PaymentHistoryPanel extends JPanel {
        ...
        public PaymentHistoryPanel(int userId) {
            ...
            summaryContainer.setBorder(new MatteBorder(0, 0, 8, 0, COLOR_BG)); // Dải phân cách xám ở đáy

            JPanel summaryPanel = new JPanel(new GridLayout(1, 3, 10, 0));
            summaryPanel.setBackground(Color.WHITE);
            summaryPanel.setBorder(new EmptyBorder(10, 20, 20, 20));

            // Tạo 3 cột tổng hợp (Sẽ được cập nhật giá trị sau khi đọc DB)
            ...
            // Lấy reference đến các thẻ JLabel chứa số tiền để lát nữa cập nhật
            ...
            // Bọc phần Summary và ScrollPane lại
        ...
        private void loadPaymentData() {
            ...
            // Lấy dữ liệu từ SERVICE thay vì query SQL trực tiếp
            List<Payment> payments =
                    paymentService.getPaymentsByUserId(loggedInUserId);
            ...
            // Update UI summary
            lblTotalCostVal.setText(
                    moneyFormat.format(totalCost)
            );

            lblTotalPaidVal.setText(
                    moneyFormat.format(totalPaid)
            );

            lblTotalPendingVal.setText(
                    moneyFormat.format(totalPending)
            );
            ...
            if (!hasData) {
                ...
                JLabel lblEmpty =
                        new JLabel("Bạn chưa có lịch sử thanh toán nào.");

                lblEmpty.setFont(
                        new Font("Segoe UI", Font.ITALIC, 15)
                );
        ...
        private JPanel createTransactionCard(String date, String methodCode, String methodText, String note, double amount) {
            JPanel card = new JPanel(new BorderLayout());
            card.setBackground(Color.WHITE);
            card.setBorder(BorderFactory.createCompoundBorder(
                    new MatteBorder(0, 0, 1, 0, new Color(240, 240, 240)), // Vạch kẻ dưới
                    new EmptyBorder(15, 20, 15, 20)
            ));
            card.setMaximumSize(new Dimension(Integer.MAX_VALUE, 90));

            // --- Hàng 1: Ngày tháng (Trên cùng) ---
            ...
            // Bên trái: Icon chữ tròn (CK / TM) + Văn bản
            JPanel leftPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 15, 0));
            leftPanel.setBackground(Color.WHITE);

            // Icon tròn tự vẽ bằng chữ để chống lỗi font
            ...
            lblIcon.setBorder(new LineBorder(iconBgColor, 1, true)); // Bo tròn

            // Phương thức và Ghi chú
            ...
            lblNote.setForeground(COLOR_TEXT_MUTED);
